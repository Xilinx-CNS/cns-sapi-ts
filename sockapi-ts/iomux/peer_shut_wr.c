...
/** @page iomux-peer_shut_wr Peer shuts down writing
 *
 * @objective Check I/O multiplexing functions behaviour when peer
 *            shuts down connection for writing.
 *
 ...
 * -# Wait for @e read event on @b iut_s socket using @p iomux
 *    function with 10 seconds timeout;
 * -# @b shutdown() server socket for writing (@c SHUT_WR);
 * -# Check that @p iomux function returns @c 1;
 * -# Check that @p iut_s socket is ready for reading;
 * -# @b recv() data from @p iut_s socket, @c 0 must be returned;
 * -# Close @b iut_s and @b tst_s sockets.
 * 
 * @author Andrew Rybchenko <Andrew.Rybchenko@oktetlabs.ru>
 */

#define TE_TEST_NAME "iomux/peer_shut_wr"
...
main(int argc, char *argv[])
{
    ...
    err = iomux_common_steps(iomux, pco_iut, iut_s, &event, IOMUX_TIMEOUT_RAND, 
                             FALSE, pco_tst, tst_s, RPC_SHUT_WR, &rc);
    if (err != 0)
    {
    ...
    if (rc != 1)
    {
        TEST_FAIL("Peer shut down its socket for writing, "
    ...
    if (event != EVT_RD)
    {
        TEST_FAIL("The socket is not reported as ready for reading only "
                  "when its peer shuts down writing");
    }

    rc = rpc_recv(pco_iut, iut_s, buffer, sizeof(buffer), 0);
    if (rc != 0)
    {
        TEST_FAIL("Not zero bytes is received from the socket "
                  "when its peer shuts down writing");
    }
