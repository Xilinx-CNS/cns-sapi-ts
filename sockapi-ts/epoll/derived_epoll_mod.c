...
/** @page epoll-derived_epoll_mod Modifying events in epfd from one process while epoll_wait(epfd) is running on another process
 *
 * @objective Check that system and epoll functions correctly handle
 *            the situation when one process modifies the events in epoll
 ...
 * -# Create @p epfd with @p iut_s socket and with @c 0 events using
 ...
 * -# Call @b epoll_ctl(@c EPOLL_CTL_MOD) to modify events for @p iut_s
 *    socket according to @p evts paramter. This call should be done on
...
#define TE_TEST_NAME  "epoll/derived_epoll_mod"
...
main(int argc, char *argv[])
{
    ...
    TEST_GET_BOOL_PARAM(event_before_mod);
    ...
    rpc_epoll_ctl_simple(pco_iut, epfd, RPC_EPOLL_CTL_ADD, iut_s, 0);

    PARSE_FUNC(func, pco_iut, pco_child, epfd, child_epfd, inst_num);

    PARSE_EVTS(evts, event, exp_ev);

    CONFIGURE_EVENTS_BEFORE(have_events, event_before_mod, evts,
    ...
    CALL_EPOLL_CTL(!wait_child, pco_child, pco_iut, child_epfd,
                   epfd, RPC_EPOLL_CTL_MOD, iut_s, event);

    TAPI_WAIT_NETWORK;

    CONFIGURE_EVENTS_AFTER(have_events, event_before_mod, evts,
    ...
    if (rc != (have_events ? 1 : 0))
    {
        TEST_VERDICT("%s_wait returned %d instead of %d",
                     iomux_call_en2str(iomux), rc, have_events ? 1 : 0);
    }
