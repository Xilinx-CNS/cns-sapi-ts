...
/** @page sockopts-bindtodevice_bond Checking data receiving via bond interface
 *
 * @objective Check that socket bound to bond interface can
 *            receive data only via its slaves
 *
 * @type conformance
 *
 ...
 *     IUT, bind it to the address assigned to created
 *     interface and to interface itself via @c
 *     SO_BINDTODEVICE socket option.
 *  -# Create @p tst_s socket of @p sock_type type on
 *     TESTER, bind it to @p tst1_addr.
 *  -# Create a connection between created pair of sockets.
 *  -# Send data from @p tst_s and check that it can be
 *     received from @p iut_s.
 *  -# Delete @p iut_if1 from created bonding interface;
 *     check that we can no longer receive from @p iut_s
 *     socket data sent from @p tst_s socket.
 *
 * @author Dmitry Izbitsky <Dmitry.Izbitsky@oktetlabs.ru>
 */

#define TE_TEST_NAME  "sockopts/bindtodevice_bond"
...
main(int argc, char *argv[])
{
    ...
    iut_s = rpc_socket(pco_iut,
                       rpc_socket_domain_by_addr(bond_addr), sock_type,
                       RPC_PROTO_DEF);
    rpc_bind(pco_iut, iut_s, bond_addr);
    namelen = sizeof(*bond_addr);
    rpc_getsockname(pco_iut, iut_s, bond_addr, &namelen);
    rpc_bind_to_device(pco_iut, iut_s, bond_ifname);

    tst_s = rpc_socket(pco_tst,
                       rpc_socket_domain_by_addr(tst1_addr), sock_type,
                       RPC_PROTO_DEF);
    if (strcmp(mode, "team4") == 0)
    ...
    {
        rpc_connect(pco_tst, tst_s, bond_addr);
        if (strcmp(mode, "team4") == 0)
            rpc_connect(pco_iut, iut_s, bond_aux_addr);
        else
    ...
    else
    {
        ...
        if (strcmp(mode, "team4") == 0)
            ...
            rpc_connect(pco_iut, iut_s, tst1_addr);
        tst_s_aux = rpc_accept(pco_tst, tst_s, NULL, NULL);
