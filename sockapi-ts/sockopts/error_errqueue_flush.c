...
/** @page sockopts-error_errqueue_flush Flushing socket error queue by disabling IP_RECVERR socket option
 *
 * @objective Check that disabling of IP_RECVERR option flushes socket error queue and
 *            clears the value of @c SO_ERROR socket option.
 *
 * @type conformance
 *
...
#define TE_TEST_NAME  "sockopts/error_errqueue_flush"
...
main(int argc, char *argv[])
{
   ...
   TAPI_WAIT_NETWORK;
    ...
    if ((rpc_errno)opt_val != RPC_EOK)
        TEST_FAIL("Step 2: After flushing error queue SO_ERROR option is set "
                  "to %s, but it is expected to be %s",
