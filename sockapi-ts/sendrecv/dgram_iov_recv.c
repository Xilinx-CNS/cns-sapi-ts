...
/** @page sendrecv-dgram_iov_recv I/O vector receive operations with datagram sockets
 *
 * @objective Check support of I/O operations with DGRAM sockets.
 *            One call to I/O vector receive operation 
 *            must receive one datagram.
 *
 * @type conformance
 *
...
#define TE_TEST_NAME  "sendrecv/dgram_iov_recv"
...
execute_pattern(int n, rpc_scatter_read_f func)
{
    ...
    {
        if (iovec_create(patterns[n].pattern, 
                         patterns[n].len, &rx) < 0)
            TEST_STOP;
        cnt = patterns[n].cnt;
    }
