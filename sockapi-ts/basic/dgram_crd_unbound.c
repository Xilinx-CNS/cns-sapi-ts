...
/** @page basic-dgram_crd_unbound Connect/reconnect/disconnect of not bound datagram socket
 *
 * @objective Connect/reconnect/disconnect UDP socket and check
 *            @b getsockname()/getpeername() output.
 *
 * @type conformance
 *
...
#define TE_TEST_NAME    "basic/dgram_crd_unbound"
...
main(int argc, char *argv[])
{
    ...
    TEST_STEP("Call @b getsockname() on @b iut_s and check that "
              ...
              "@p iut_addr1 (not considering port number).");
    memset(&name, 0, sizeof(name));
    namelen = sizeof(name);
    rpc_getsockname(pco_iut, iut_s, SA(&name), &namelen);
    if (te_sockaddrcmp_no_ports(iut_addr1, te_sockaddr_get_size(iut_addr1),
                                SA(&name), namelen))
    {
        TEST_VERDICT("getsockname() returned unexpected address "
                     "after connect() to tst1_addr");
    }
    memset(&wild_addr, 0, sizeof(wild_addr));
    wild_addr.ss_family = iut_addr1->sa_family;
    te_sockaddr_set_wildcard(SA(&wild_addr));
    te_sockaddr_set_port(SA(&wild_addr),
                         te_sockaddr_get_port(CONST_SA(&name)));
    te_sockaddr_set_port(SA(iut_addr1),
                         te_sockaddr_get_port(CONST_SA(&name)));
    te_sockaddr_set_port(SA(iut_addr2),
                         te_sockaddr_get_port(CONST_SA(&name)));

    TEST_STEP("Call @b getpeername() on @b iut_s and check that "
              "the function returns address @p tst1_addr.");
    namelen = sizeof(name);
    rpc_getpeername(pco_iut, iut_s, SA(&name), &namelen);
    if (te_sockaddrcmp(SA(&name), namelen, tst1_addr,
                       te_sockaddr_get_size(tst1_addr)) != 0)
    {
        TEST_VERDICT("getpeername() returned unexpected address "
                     "after connect() to tst1_addr");
    }
    ...
    {
        RING("Reconnect to the peer from another subnet does "
    ...
    namelen = sizeof(name);
    rpc_getpeername(pco_iut, iut_s, SA(&name), &namelen);
    if (te_sockaddrcmp(SA(&name), namelen, tst2_addr,
                       te_sockaddr_get_size(tst2_addr)) != 0)
    {
        TEST_VERDICT("getpeername() returned unexpected address "
                     "after connect() to tst2_addr");
    }
    ...
    TEST_STEP("Call @b getsockname() on @b iut_s and check that "
              "the function returns wildcard address.");
    memset(&name, 0, sizeof(name));
    namelen = sizeof(name);
    rpc_getsockname(pco_iut, iut_s, SA(&name), &namelen);
    ...
    {
        RING("Disconnect resets local port to zero");
    }
    ...
    TEST_STEP("Call @b getpeername() on @b iut_s and check "
              "that the functions returns @c -1 and @b errno on @p pco_iut "
    ...
    if (rc != -1)
    {
        TEST_VERDICT("getpeername() succeeded after connecting to "
                     "AF_UNSPEC address");
    }
    else
    {
