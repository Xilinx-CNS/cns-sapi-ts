...
/** @page basic-threads_client Stream client socket in multiple threads
 *
 * @objective Check that same stream client socket may be accessed
 *            from different threads and action in any thread changes
 *            the state of socket for other thread.
 *
 ...
 * -# Perform routine #sockts_get_socket_state for @p iut_s from @p pco_iut1.
 * -# Perform routine #sockts_get_socket_state for @p iut_s from @p pco_iut2.
 * -# Check that both obtained states of @p iut_s on both threads are the
 *    @c STATE_CONNECTED.
 * -# Call @b connect() for @p iut_s on @p pco_iut2.
 * -# Check that @b connect() return -1 with @b errno @c EISCONN.
 * -# Check that local and remote socket address of @p iut_s is the same
...
#define TE_TEST_NAME  "basic/threads_client"
...
main(int argc, char *argv[])
{
    ...
    CHECK_SOCKET_STATE(pco_iut1, iut_s, pco_tst, acc_s, STATE_CONNECTED);
    CHECK_SOCKET_STATE(pco_iut2, iut_s, pco_tst, acc_s, STATE_CONNECTED);

    RPC_AWAIT_IUT_ERROR(pco_iut2);
    rc = rpc_connect(pco_iut2, iut_s, tst_addr);
    if (rc != -1)
    {
        TEST_FAIL("Call of connect() on already connected from another "
                  "thread socket returns %d instead of -1", rc);
    }
    CHECK_RPC_ERRNO(pco_iut2, RPC_EISCONN,
                    "Call of connect() on already connected from "
                    "another thread socket returns -1, but");
    ...
    CHECK_SOCKET_STATE(pco_iut2, iut_s, pco_tst, acc_s, STATE_SHUT_RD);
...
cleanup:
    ...
    if (iut_s != -1)
    {
        int iut_s_copy = iut_s;

        RPC_CLOSE(pco_iut1, iut_s);
        /* FIXME */
        CHECK_SOCKET_STATE(pco_iut2, iut_s_copy, NULL, -1, STATE_CLOSED);
    }
