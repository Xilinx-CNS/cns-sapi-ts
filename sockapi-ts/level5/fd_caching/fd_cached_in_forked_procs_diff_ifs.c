...
/** @page fd_caching-fd_cached_in_forked_procs_diff_ifs 
 *
 * @objective Check that sockets can be cached/reused between different
 *            processes and different interfaces.
 *            Check that FDs don't leak.
 *
 * @type conformance
 *
...
#define TE_TEST_NAME    "level5/fd_caching/fd_cached_in_forked_procs_diff_ifs"
 ...
 */ 
typedef struct socket_info_fd_cached_in_forked_procs_diff_ifs {
 ...
 */ 
typedef struct conn_s_info_fd_cached_in_forked_procs_diff_ifs {
 ...
 */ 
typedef struct p_info_fd_cached_in_forked_procs_diff_ifs {
...
#define ADDR_TO_CONNECT_TO p_current->conn_s[j].related_info->to
    ...
    {    
        for (i = 0, p_current = p_list; i < p_number; i++, p_current++)
        {
            p_current->state = (uint16_t)(random() % 0xffff) + 1;

            for (j = 0; j < MAX_CONNECTIONS_PER_PROCCESS; j++)
            {
                if (!((1 << j) & p_current->state))
                {
                    continue;
                }
