...
/** @page aio-aio_write_many_socks  Many AIO write requests on different sockets
 *
 * @objective Check that several AIO write requests may be posted on
...
#define TE_TEST_NAME  "aio/aio_write_many_socks"
...
{   
    ...
    for (i = 0; i < sock_num; i++)
    {
        ...
        reqs[i].buf = rpc_malloc(pco_iut, reqs[i].len);
        rpc_set_buf(pco_iut, tx_buf, reqs[i].len, reqs[i].buf);
        
        /* Create and fill aiocb */
        reqs[i].cb = rpc_create_aiocb(pco_iut);
        rpc_fill_aiocb(pco_iut, reqs[i].cb, conns[i].iut_s, 0, 0,
                       reqs[i].buf, reqs[i].len, &reqs[i].ev);
    ...
    for (i = 0; i < sock_num; i++)
    {
        len = rpc_recv(pco_tst, conns[i].tst_s, tx_buf, reqs[i].len, 0);
        if ((int)len != reqs[i].len)
            TEST_FAIL("%d recv() returned %d instead %d", i + 1, len,
                      reqs[i].len);
        
        rpc_get_buf(pco_iut, reqs[i].buf, reqs[i].len, rx_buf);
        if (memcmp(tx_buf, rx_buf, reqs[i].len) != 0)
        TEST_FAIL("Data sent from the TST do not match data received "
                  "on the IUT in %d connection", i + 1);
    }
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if (req->notify)
            TEST_FAIL("Completion notification is received twice (request"
                      "on socket %d: %s, notification %s%s)", list[i].val,
            ...
            case RPC_SIGEV_THREAD:
                                                 ...
                                                 strlen(AIO_CALLBACK_NAME)))
                {
                    TEST_FAIL("Incorrect callback function " 
                              ...
                              "on socket %d: %s, notification %s%s)",
                              list[i].callback_num, list[i].val,
                              conns[(list[i].val)].iut_s,
                              sigev_notify_rpc2str(req->ev.notify));
                }
                break;
            ...
            case RPC_SIGEV_SIGNAL:
                if (list[i].signo == 0)
                {
                    TEST_FAIL("Completion is notified using callback "
                              "instead signal (request on socket %d:"
                              "%s, notification %s%s)", list[i].val,
                              conns[(list[i].val)].iut_s,
                              sigev_notify_rpc2str(req->ev.notify));
                }
                ...
                if (list[i].signo != req->ev.signo)
                {
                    TEST_FAIL("Unexpected signal is received: %s "
                              "instead %s (request on socket %d:"
                              "%s, notification %s%s)", 
                              signum_rpc2str(list[i].signo),
                              signum_rpc2str(req->ev.signo), list[i].val,
                              conns[(list[i].val)].iut_s,
                              sigev_notify_rpc2str(req->ev.notify));
                }
                break;
    ...
    for (i = 0, req = reqs; i < sock_num; i++, req++)
    {
        if ((rc = rpc_aio_error(pco_iut, req->cb)) != 0)
            TEST_FAIL("aio_error() returned %r instead 0 (request on"
                      "socket %d: %s, notification %s%s)", 
                      rc, list[i].val, conns[(list[i].val)].iut_s,
                      sigev_notify_rpc2str(req->ev.notify));
    
        if ((rc = rpc_aio_return(pco_iut, req->cb)) <= 0 ||
             rc != req->len)
        {
            TEST_FAIL("aio_return() returned %d instead %d (request on"
                      "socket %d: %s, notification %s%s)", 
                      rc, req->len, list[i].val,
                      conns[(list[i].val)].iut_s,
                      sigev_notify_rpc2str(req->ev.notify));
        }
        
        if (req->ev.notify == RPC_SIGEV_THREAD && !req->notify)
            TEST_FAIL("No completion notification is received (request on"
                      "socket %d: %s, notification %s%s)",
                      list[i].val, conns[(list[i].val)].iut_s,
                      sigev_notify_rpc2str(req->ev.notify));
