...
/** @page aio-aio_read_ignore_opcode  Check that lio_opcode is ignored by aio_read()
 *
 ...
 *    @a aio_lio_opcode should be set to @c LIO_WRITE.
 * -# Check that socket @p tst_s is not readable.
 * -# Send data via @p tst_s.
...
#define TE_TEST_NAME  "aio/aio_read_ignore_opcode"
...
main(int argc, char *argv[])
{
    ...
    rpc_fill_aiocb(pco_iut, cb, iut_s, RPC_LIO_WRITE, 0, buf,
                   DATA_BULK + 1, &ev);
