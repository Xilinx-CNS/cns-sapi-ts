...
/*
 * Test attacks/ip/teardrop
 * Fragments with incorrect offsets
 */

/** @page ip-teardrop  Fragments with overlapped offsets
 *
 * @objective Emulate "Teardrop" attack.
 *
 ...
 *    to stream connection created on the step 1) splitted to 2-10 overlapped
 *    fragments. Destination ports for TCP SYN packets and UDP packets
 *    should be unused.
...
#define TE_TEST_NAME  "attacks/ip/teardrop"
...
#define MIN_PKT_LEN     2048
#define MAX_PKT_LEN     4096
#define PKT_NUM         (1024 * 1024 * 16) /**< Number of packets for flooding */
...
create_template(pkt_type type)
{
    ...
    {
        case PKT_TCP_SYN:
            CHECK_RC(tapi_tcp_make_msg(SIN(iut_addr)->sin_port,
        ...
        case PKT_UDP:
            ...
            *(uint16_t *)(buf + 2) = SIN(iut_addr)->sin_port + 1;
            buf[4] = len >> 8;
            buf[5] = len & 0xFF;
            break;
    }
    ...
    for (i = 0; i < n; i++)
    {
        ...
        tmp->real_length += rand_range(1, FRAG_LEN);
...
main(int argc, char *argv[])
{
    ...
    iut_addr1 = *SIN(iut_addr);
    TAPI_SET_NEW_PORT(pco_iut, &iut_addr1);
    tst_addr1 = *SIN(tst_addr);
    ...
    /* Create CSAP for sending UDP packets with overlapped fragments */
    ...
    /* Create CSAP for sending of TCP data packets with overlapped fragments */
    ...
    /* Create CSAP for sending of TCP SYN packets with overlapped fragments */
    ...
    udp_pkt = create_template(PKT_UDP);
    tcp_syn_pkt = create_template(PKT_TCP_SYN);
    tcp_data_pkt = create_template(PKT_TCP_DATA);
    ...
    /* Data are sent and acked */
    tapi_tcp_update_sent_seq(tcp_conn, sizeof(tx_buf));

    if (num != sizeof(tx_buf))
        TEST_FAIL("Unexpected number of bytes %d is received", num);
...
cleanup:
    ...
    if (csap_tcp_data != CSAP_INVALID_HANDLE)
        CLEANUP_CHECK_RC(tapi_tad_csap_destroy(pco_tst->ta, 0, csap_tcp_data));
