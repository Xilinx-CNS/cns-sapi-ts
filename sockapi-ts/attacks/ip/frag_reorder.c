...
/* 
 * Test attacks/ip/frag_reorder  
 * Re-ordered fragments
 */

/** @page ip-frag_reorder  Re-ordered fragments
 *
 * @objective Check that re-ordering of fragments does not lead to packets
 *            loss.
 *
 * @param env       Testing environment:
 *                  - @ref arg_types_env_peer2peer
 * @param min_pkt   Minimum packet length (in triples with @p max_pkt and
 *                  @p frag_len):
 *                  - 100
 *                  - 2000
 *                  - 5000
 ...
 *    UDP datagrams splitted to fragments; random number of fragment
 *    pairs in each datagram should be re-ordered.
 ...
 *    on the @p pco_tst is equal to number of received datagrams and
 *    bytes on the @p pco_iut.
 *
 * @author Elena Vengerova <Elena.Vengerova@oktetlabs.ru>
 */

#define TE_TEST_NAME  "attacks/ip/frag_reorder"
...
#define PKT_NUM       8 //(1024 * 32) /**< Number of packets for flooding */
#define UDP_HDR_LEN   8           /**< TCP header length in bytes */
...
create_template(int *payload_len)
{
    ...
    tapi_ip_frag_spec  frags[MAX_FRAG_NUM];
    tapi_ip_frag_spec  frags_mixed[MAX_FRAG_NUM];
    ...
    *(uint16_t *)(buf + 2) = SIN(iut_addr)->sin_port;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        while (frags[k].real_length == 0)
            k = (k + 1) % n;
            
        frags_mixed[i] = frags[k];
        frags[k].real_length = 0;
    }

    rc = tapi_ip4_template(frags_mixed, n, 1, IPPROTO_UDP, buf,
                           UDP_HDR_LEN, &result);
    if (rc != 0)
