...
/* 
 * Test attacks/udp/land  
 * UDP packets with same src/dst addresses/ports
 */

/** @page udp-land  UDP packets with same src/dst addresses/ports
 *
 * @objective Check that flood of UDP packets with the same src/dst
 ...
 *    UDP packets with same source/destination ports and
...
#define TE_TEST_NAME  "attacks/udp/land"
...
main(int argc, char *argv[])
{
    ...
    sprintf(buf, 
             "{ arg-sets { simple-for:{begin 0, end %u} },           "
             "  pdus  {                                              "
