...
/* 
 * Test attacks/icmp/icmp_forged_acknum
 * 
 */

/** @page icmp-icmp_forged_acknum  Send ICMP packet with forged ack number
 *
 * @objective Check that port unreachable ICMP packets with incorrect ACK 
 ...
 *
 * @par Scenario
 * -# Start sniffing interface of @p pco_tst1, wait for SYN packet
 *    from @p pco_iut.
 * -# Send a SYN packet (i.e. try to  connect) from @p pco_iut 
 *    to @p pco_tst1. 
 * -# Send forged ICMP message with incorrect ACK number to @p pco_iut.
...
#define TE_TEST_NAME  "attacks/icmp/icmp_forged_acknum"
...
construct_icmp(uint8_t *payload, uint16_t pld_len)
{
    ...
    ((struct tcphdr *)(buf + ICMP_HDR_LEN))->ack_seq = 0xabcd;
...
main(int argc, char **argv)
{
    ...
    struct icmp_forged_acknum_cb_flag ud;
...
cleanup:
    ...
    {
        WARN("It seems that icmp_forged_acknum made TA crasy");
