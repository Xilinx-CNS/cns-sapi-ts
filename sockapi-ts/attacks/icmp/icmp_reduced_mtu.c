...
/* 
 * Test attacks/icmp/icmp_reduced_mtu
 * 
 */

/** @page icmp-icmp_reduced_mtu Send ICMP packet with reduced Path MTU.
 *
 * @objective Check that port unreachable ICMP packets with reduced 
 *            PATH MTU does not lead to denial of service.
 *
 ...
 *    @p pco_iut.
 * -# Send a SYN packet (i.e. try to  connect) with forged destination 
 *    address from @p pco_iut to @p pco_tst1. 
 * -# Send forged ICMP message with reduced Path MTU to @p pco_iut.
...
#define TE_TEST_NAME  "attacks/icmp/icmp_reduced_mtu"
...
construct_icmp(uint8_t *payload, uint16_t pld_len)
{
    ...
    if (((struct icmphdr *)buf)->un.frag.mtu > 0)
        ((struct icmphdr *)buf)->un.frag.mtu--;
...
main(int argc, char **argv)
{
    ...
    struct icmp_reduced_mtu_cb_flag ud;
...
cleanup:
    ...
    {
        WARN("It seems that icmp_reduced_mtu made TA crasy");
