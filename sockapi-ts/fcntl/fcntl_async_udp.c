...
/** @page fcntl-fcntl_async_udp Usage of fcntl() functionality on UDP socket
 *
 * @objective Check possibilty provided by @b fcntl() for signal controlled
 *            input/output on UDP sockets.
 *
 * @type conformance
 *
...
#define TE_TEST_NAME  "fcntl/fcntl_async_udp"
...
main(int argc, char *argv[])
{
    ...
    TEST_GET_STRING_PARAM(use_connect);

    memset(&foex, 0, sizeof(foex));
    pco_iut_pid = rpc_getpid(pco_iut);

    CHECK_NOT_NULL(buf = sockts_make_buf_stream(&buf_len));

    CHECK_RC(tapi_sigaction_simple(pco_iut, RPC_SIGIO,
                                   SIGNAL_REGISTRAR, &old_act));
    restore_signal_handler = TRUE;

    TEST_STEP("Create two @c SOCK_DGRAM socktes: @p iut_s on @p pco_iut and @p tst_s "
              "on @p pco_tst");
    ...
    TEST_STEP("Call @c connect() on @p tst_s socket");
    rpc_connect(pco_tst, tst_s, iut_addr);

    TEST_STEP("Call @c connect() on @p iut_s socket if @p use_connect is "
              "@c before");
    if (strcmp(use_connect, "before") == 0)
    ...
    TEST_STEP("Call @c connect() on @p iut_s socket if @p use_connect is "
              "@c after");
    if (strcmp(use_connect, "after") == 0)
        rpc_connect(pco_iut, iut_s, tst_addr);

    TEST_STEP("Send some data from @p tst_s socket");
    RPC_SEND(rc, pco_tst, tst_s, buf, buf_len, 0);
