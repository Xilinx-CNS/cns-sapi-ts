...
/** @page sendfile-f2s_flooder2one Performing 'file to socket' sendfile() operation together with some TCP traffic to the common socket
 *
 * @objective Check a possibility of copying of the file to the socket
 *            by means of @b sendfile() system call together with some TCP
 *            traffic existing on the same socket.
 *
 * @type conformance
 *
 ...
 *    if @p FALSE.
 * -# Run @ref iomux-flooder on @p pco_iut and @ref iomux-echoer on
 *    @p pco_tst PCO with the following parameters:
 *      - @p pco_iut, { @p iut_s }, { @p iut_s }, @c 1000,
 *        @c time2run, @b select();
...
#define TE_TEST_NAME  "sendfile/f2s_flooder2one"
...
main(int argc, char *argv[])
{
    ...
    if (rpc_iomux_echoer(pco_tst, &tst_s, 1, time2run + 1, FUNC_POLL,
                         &echo_tx, &echo_rx) != 0)
    {
    ...
    if (((echo_rx - flooder_tx) != (unsigned long)file_length) ||
        (echo_tx != flooder_rx))
    {
        ERROR("Traffic statistics: flooder_tx=%u, flooder_rx=%u, "
             "echo_tx=%u, echo_rx=%u",
...
cleanup:
    if (flooder_started && pco_iut != NULL)
    {
        flooder_started = FALSE;
        pco_iut->op = RCF_RPC_WAIT;
        if (rpc_iomux_flooder(pco_iut, &iut_s, 1, &iut_s, 1, 1000, time2run, 5,
                              FUNC_POLL, &flooder_tx, &flooder_rx) != 0)
        {
            ERROR("Unexpected rpc_iomux_flooder() failure on pco_iut");
            result = EXIT_FAILURE;
        }
    }
